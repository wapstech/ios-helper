...
- (void)nextMusic
{
    ...
    NSString *str=[[NSUserDefaults standardUserDefaults] objectForKey:@"paly"];
    NSString *str1=[[NSUserDefaults standardUserDefaults] objectForKey:@"not"];

    if ([str1 isEqualToString:@"手动"]) { //点击下一曲
       
        self.currentIndex ++;

    }else if([str1 isEqualToString:@"自动"]){
        
        if ([str isEqualToString:@"顺序"]) {  //顺序播放++ 单曲不需要做处理
